...
case "${command[0]}" in
	...
	"getfile")
		...
		echo "24/${command[1]}/${command[2]}" >> "./output/$FILE"
		;;
	"adduser")
		# Add user.
		if ((${#command[@]} != 2)); then
	...
	"fromgroup")
		...
		echo "2D/${command[1]}/${command[2]}" >> "./output/$FILE"
		;;
	"getusers")
		# Get list of users.
		if ((${#command[@]} != 2)); then
	...
	"getletterindex")
		...
		echo "3B/${command[1]}/${command[2]}" >> "./output/$FILE"
		;;
	"kmsg")
	...
	"smc_skipif")
		...
		echo "3E/${command[1]}/${command[2]}" >> "./output/$FILE"
		;;
	"smc_if")
		# SMC interpreting - if.
		if ((${#command[@]} != 4)); then
